# NutuWebWallet - a lightweight web wallet for Nutucoin based on OpenMonero/Nutu 

In this example [restbed](https://github.com/Corvusoft/restbed/) is used to
demonstrate how to provide Nutu related JSON REST service. For this purpose,
 a service called NutuWebWallet was developed.


NutuWebWallet is an open source implementation of backend of
https://mymonero.com/ adapted for Nutu. The frontend, which includes HTML, CSS, JavaScript, was adapted
from (and originally developed by) https://mymonero.com/.

NutuWebWallet's backend is open sourced, free
...
to MyMonero. They include:
 ...
 - the wallets generated use 25 word mnemonics, fully compatible with official Nutu wallets
(13 word mnemonics generated by MyMonero work as usual though).
 ...
 - private tx key given to a user which can be used to prove sending LOKI to a recipient.
 - Cross-Origin Resource Sharing (CORS) disabled.
 - transaction confirmation window added to double check tx details before its committed.
...
![NutuWebWallet](screenshot/screen1.png)


## Host it yourself

The NutuWebWallet consists of four components that need to be setup for it to work:
 ...
 - Nutu daemon - daemon must be running and fully sync, as this is
 where all transaction data is fetched from and used. Daemon also commits txs
 from the NutuWebWallet into the Nutu network.
 - Backend - fully written in C++. It uses [restbed](https://github.com/Corvusoft/restbed/) to serve JSON REST to the frontend
 and [mysql++](http://www.tangentsoft.net/mysql++/) to interface the database. It also accesses Nutu blockchain and "talks"
 to Nutu daemon.
...
NutuWebWallet is not as fast as MyMonero.
 ...
 blocks, complex database structures and SQL queries. Also, no ongoing monitoring of user's
  ...
  user logs in (viewkey and address are submitted to the search thread), and finish shortly
...
Below are example and basic instructions on how to setup up and run NutuWebWallet on Ubuntu 16.04.
For other Linux operating systems, the instructions are analogical.


#### Nutu download and compilation

Download and compile recent Nutu into your home folder:

```bash
# first install Nutu dependencies
sudo apt update
...
git clone --recursive https://github.com/nutu-project/nutu

cd nutu/

# checkout last Nutu version
git checkout -b last_release 0.2.0

make
```

#### Compilation of the NutuWebWallet (don't run it yet)

Once Nutu was downloaded and compiled, we can download NutuWebWallet and compile it.
...
work without database, setup frontend, and synced and running Nutu blockchain.

```bash
# need mysql++ libraries
sudo apt install libmysql++-dev

# go to home folder if still in ~/nutu
cd ~

git clone https://github.com/nutu-project/nutu-locker.git
...
# altearnatively can use cmake -DLOKI_DIR=/path/to/nutu_folder ..
# if Nutu is not in ~/nutu
...
Download `db.sql` provided and setup the `nutu_wallet` database. `db.sql` script will
drop current `nutu_wallet` if exist. So don't run it, if you have already some important information
in the `nutu_wallet` database.

Assuming we are still in `build` folder:

```bash
# apply it to mysql
mysql -p -u root < ../sql/db.sql
```
...
```bash
sudo mkdir /var/www/html/nutu-locker
sudo cp -rvf ../html/* /var/www/html/nutu-locker/
```

Setup document root in `lighttpd.conf` into nutu-locker folder
...
```bash
server.document-root    = "/var/www/html/nutu-locker"
```
...
Before running `nutu-locker`:
 ...
 - make sure Nutu daemon is running and fully sync. If using testnet or stagenet networks, use Nutu daemon
...
```bash
./nutu-locker -s -c /path/to/config.json
```


## NutuWebWallet JSON REST API

Example JSON REST requests and their responses of [NutuWebWallet](https://github.com/nutu-project/nutu-locker) are provided below. The long term goal
is to make the api conform to [jsent](https://labs.omniti.com/labs/jsend)
specification which describes successful, failed and error responses. At present,
the NutuWebWallet API does not fully conform to that.

#### get_version

Get version of the NutuWebWallet, its API and Nutu.
...
```json
{
  ...
  "nutu_version_full": "0.2.0-master"
}
```
...
Login an existing or new user into NutuWebWallet
...
Get the list of all possible spendings. Used when calculated the wallet balance.
...
Get the list of all outputs with key images which could mean that the outputs
...
Get random outputs from the Nutu daemon to be used as ring members
...
Submit raw tx in hex generated by the frontend to be relayed to the Nutu network.
